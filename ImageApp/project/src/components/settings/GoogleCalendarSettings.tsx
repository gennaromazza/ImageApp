...
const GoogleCalendarSettings: React.FC = () => {
  ...
  const handleSync = async () => {
    ...
    if (!auth.currentUser) {
      ...
      console.log('handleSync: no currentUser, returning');
      return;
    }
    try {
      setSyncing(true);
      console.log('handleSync: calling syncAllBookingsToGoogleCalendar with userId =', auth.currentUser.uid);
      const syncResult: DetailedSyncResult = await syncAllBookingsToGoogleCalendar(auth.currentUser.uid);
      console.log('handleSync: syncAllBookingsToGoogleCalendar result =', syncResult);
      setSuccess(true);
      setSyncDetails(syncResult);
