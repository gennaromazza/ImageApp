import React, {
  ...
  function Alert({
    ...
    return (
      <div
        className={`relative w-full rounded-lg border p-4 text-sm ${variantClasses} ${className || ''}`}
        {...props}
      >
        {children}
      </div>
  ...
  function PopoverContent({ className, children, ...props }: PopoverContentProps) {
    ...
    return (
      <div
        className={`absolute z-50 mt-2 rounded-md border bg-white p-2 shadow ${className || ''}`}
  ...
  function NotifyAppWhtz({ booking }: NotifyAppWhtzProps): JSX.Element {
    ...
    const [message, setMessage] = useState<string>(
      `Ciao ${booking.firstName || 'Cliente'} 👋, ti ricordiamo il tuo appuntamento il ${booking.booking_date} ⏰ alle ${booking.booking_time}.`
    );
    ...
    const sendReminder = useCallback(async (): Promise<void> => {
      ...
      try {
        if (!booking?.id || !booking?.phone || !booking?.booking_date || !booking?.booking_time) {
        ...
        // Se vuoi fare multiple prove senza bloccare il bottone:
        // 1) RIMUOVI questo setSent(true) e
        // 2) RIMUOVI la logica di updateDoc se non vuoi segnare come inviato su Firestore
        //
        // Rimuovendo questi passaggi, il pulsante rimarrà sempre cliccabile.
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <button
            ...
            className={`
              ...
              ${sent ? 'bg-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <DialogFooter className="flex space-x-2 sm:space-x-0 mt-4">
            ...
            <Button type="button" onClick={sendReminder} disabled={loading} className="relative">
              {loading && (
                <MessageCircle className="absolute left-2 animate-spin" size={16} />
              )}
