...
import { Heart, Check, AlertCircle, Film, Share2, Download, X } from 'lucide-react';
import { db } from '../../lib/firebase';
import { collection, query, where, getDocs, addDoc, orderBy, doc, getDoc, updateDoc, serverTimestamp } from 'firebase/firestore';
...
const GalleryViewer = () => {
    ...
    const loadGallery = async () => {
      ...
      try {
        ...
        if (!selectionsSnapshot.empty) {
          const selectionData = selectionsSnapshot.docs[0].data();
          setSelectedPhotos(selectionData.photoIds || []);
  ...
  const handleSaveSelection = async () => {
    ...
    try {
      ...
      if (!selectionsSnapshot.empty) {
        const selectionDoc = selectionsSnapshot.docs[0];
  ...
  onClick={() => {
    console.log('Cliccato su foto:', photo.id); // Debug
...
>
  <Heart size={24} className={selectedPhotos.includes(photo.id) ? 'fill-current' : ''} />
</button>
        ...
        <AnimatePresence>
          {currentPhotoIndex !== null && (
            <PhotoViewer
              ...
              onClose={() => {
                console.log('Chiuso PhotoViewer'); // Debug
                setCurrentPhotoIndex(null);
              }}
              onNext={() => {
                console.log('Cliccato su Avanti'); // Debug
                setCurrentPhotoIndex(prev => 
                  prev !== null && prev < photos.length - 1 ? prev + 1 : prev
                );
              }}
              onPrev={() => {
                console.log('Cliccato su Indietro'); // Debug
                setCurrentPhotoIndex(prev => 
