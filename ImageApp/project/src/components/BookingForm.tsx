...
const BookingForm: React.FC<BookingFormProps> = ({ existingBookings = [] }) => {
  ...
  const handleSlotSelect = (date: Date, time: string) => {
    // Creiamo la data in formato locale
  ...
  const onSubmit = async (data: BookingFormData) => {
    ...
    // Creiamo la data di inizio prenotazione in locale
    ...
    try {
      ...
      // Imposta lo stato predefinito: se nel settings sono definiti bookingStatuses, prendi il primo; altrimenti "pending"
      const defaultStatus =
        settings.bookingStatuses && settings.bookingStatuses.length > 0
          ? settings.bookingStatuses[0].id
          : 'pending';
      ...
      await addDoc(collection(db, 'bookings'), {
        ...
        booking_date: bookingStart.toISOString().split('T')[0],
  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <form onSubmit={handleFormSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Feedback sullo stato predefinito */}
