    ...
    if (bookingData.userId) {
      try {
        ...
        if (userData?.calendarConnected) {
          ...
          const startDateTime = new Date(`${formattedDate}T${bookingData.booking_time}`);
...
export const syncAllBookingsToGoogleCalendar = async (userId: string) => {
  try {
    console.log("🔄 Inizio sincronizzazione di tutte le prenotazioni per il fotografo...");
    const bookingsRef = collection(db, 'bookings');
    const q = query(bookingsRef, orderBy('created_at', 'desc'));
    const snapshot = await getDocs(q);
    ...
    for (const booking of bookings) {
      ...
      const endDateTime = new Date(startDateTime.getTime() + 60 * 60 * 1000); // Durata di 1 ora

      await addToGoogleCalendar(userId, {
        summary: `Prenotazione Fotografica - ${booking.firstName} ${booking.lastName}`,
        description: `Prenotazione: ${booking.ticket_number}\nServizio: ${booking.service_type}`,
        startDateTime: startDateTime.toISOString(),
        endDateTime: endDateTime.toISOString()
      });

      console.log(`✅ Evento sincronizzato: ${booking.firstName} ${booking.lastName} - ${booking.booking_date}`);
    }
  } catch (error) {
