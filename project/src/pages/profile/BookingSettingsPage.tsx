...
import { Link, Calendar, Globe, Image, Palette, AlertCircle, Check, Copy } from 'lucide-react';
...
const BookingSettingsPage: React.FC = () => {
  ...
  const copyBookingUrl = async () => {
    ...
    const url = `${window.location.origin}/b/${settings.customUrl}`;
    try {
            ...
            <div className="flex gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-[140px] p-2 bg-gray-700 border border-gray-600 rounded text-white"
      ...
      {settings?.customUrl && (
        <div className="bg-gray-800 rounded-lg p-6">
          <h3 className="text-xl font-semibold text-white mb-4">
            Anteprima URL Prenotazione
          </h3>
          <div className="bg-gray-700 p-4 rounded-lg">
            <p className="text-gray-300 break-all">
              {window.location.origin}/b/{settings.customUrl}
            </p>
          </div>
          <p className="text-gray-400 mt-4">
            Condividi questo URL con i tuoi clienti per permettere loro di prenotare direttamente.
          </p>
        </div>
      )}
    </motion.div>
